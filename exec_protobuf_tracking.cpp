...
std::map<bus_stop*, int> bus_stop_map;
...
static void translate_bus_route(data::BusRoute *model, bus_route *data) {
    ...
    for(unsigned int i=0; i<data->routes.size(); i++) {
        ...
        model->add_routes_index(bus_stop_map[current]);
    }
}
...
static void translate_bus_group(data::BusGroup *model, bus_group *data) {
    ...
    for(unsigned int i=0; i<data->groups.size(); i++) {
        ...
        for(unsigned int j=0; j<current->routes.size(); j++) {
            bus_stop_detail *current_stop = (bus_stop_detail*)current->routes[j];
            if(bus_stop_map.find(current_stop)!=bus_stop_map.end()) continue;
            model_bus_stop = model->add_bus_stop_sequence();
            bus_stop_map[current_stop] = bus_stop_map.size();
            translate_bus_stop(model_bus_stop, current_stop);
        }
    }
...
std::vector<bus_stop*> bus_stop_vector;
...
static bus_route* restore_bus_route(data::BusRoute *model) {
    ...
    for(unsigned i=0; i<model->routes_index_size(); i++) {
        auto model_bus_stop = model->routes_index(i);
        bus_stop* bstop = bus_stop_vector[model_bus_stop];
...
static bus_group* restore_bus_group(data::BusGroup *model) {
    ...
    for(unsigned i=0; i<model->bus_stop_sequence_size(); i++) {
        auto model_bus_stop = model->bus_stop_sequence(i);
        bus_stop_detail* bstop = restore_bus_stop(&model_bus_stop);
        bus_stop_vector.push_back(bstop);
    }
