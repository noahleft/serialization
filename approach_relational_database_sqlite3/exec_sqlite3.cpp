...
static void add_database_schema(sqlite3 *db) {
    ...
    int rc = sqlite3_exec(db, schema.c_str(), 0, 0, 0);
}

static void insert_database(sqlite3 *db, ROOT_TYPE *root) {
    int rc;
    int bs_idx = 1;
    int br_idx = 1;
    for(unsigned i=0; i<root->groups.size(); i++) {
        ...
        for(unsigned j=0; j<root->groups[i]->routes.size(); j++) {
            ...
            sql << ");";
            rc = sqlite3_exec(db, sql.str().c_str(), 0, 0, 0);
        }
        ...
        br_sql << ");";
        rc = sqlite3_exec(db, br_sql.str().c_str(), 0, 0, 0);
    }
}
...
std::string serialize(ROOT_TYPE *root) {
    ...
    sqlite3 *db;
    int rc = sqlite3_open(buf.c_str(), &db);
    add_database_schema(db);
    insert_database(db, root);
    sqlite3_close(db);
    return buf;
}
...
static int bs_callback(void *data, int argc, char **argv, char **azColName){
    double lat, lng;
    lat = atof(argv[1]);
    lng = atof(argv[2]);
...
static int br_callback(void *data, int argc, char **argv, char **azColName){
...
ROOT_TYPE* restore_database(sqlite3 *db) {
    std::string bs = "SELECT NAME, LAT, LNG FROM BUS_STOP;";
    int rc = sqlite3_exec(db, bs.c_str(), bs_callback, 0, 0);
    std::string br = "SELECT NAME, ROUTE FROM BUS_ROUTE;";
    rc = sqlite3_exec(db, br.c_str(), br_callback, 0, 0);
...
ROOT_TYPE* deserialize(std::string buf) {
    sqlite3 *db;
    int rc = sqlite3_open(buf.c_str(), &db);
    ROOT_TYPE *root = restore_database(db);
    sqlite3_close(db);
    return root;
}
